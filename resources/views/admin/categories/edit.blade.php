...
@section('content')
    ...
    <div class="col-md-12 col-12">
        ...
        <div class="card">
            ...
            <div class="card-body">
                <form id="jquery-val-form" method="post" action="{{route('products.update',$product->id)}}" enctype="multipart/form-data">
                    @csrf
                    @method('put')
                    ...
                    <div class="row">
                        <div class="mb-1 col-md-6">
                            ...
                            <input type="text" class="form-control" id="product_name" name="product_name" value="{{old('product_name',$product->product_name)}}" placeholder="اسم المنتج - مثال: شيبسي" required/>
                        </div>

                        <div class="mb-1 col-md-6">
                            <label class="form-label" for="product_code">كود المنتج</label>
                            <input type="text" class="form-control" id="product_code" name="product_code" value="{{old('product_code',$product->product_code)}}" placeholder="" required/>
                        </div>
                        ...
                        <div class="mb-1 col-md-6">
                            ...
                            <select class="form-select select2" id="product_status_id" name="product_status_id" required>
                                ...
                                @foreach(\App\Http\Controllers\LookupController::getLookupData('ProductStatus') as $status)
                                    <option value="{{$status->id}}" {{$product->product_status_id == $status->id ? 'selected':'' }}>{{$status->status_name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-1 col-md-6">
                            ...
                            <select class="form-select select2" id="product_accept_id" name="product_accept_id" required>
                                ...
                                @foreach(\App\Http\Controllers\LookupController::getLookupData('ProductAccept') as $status)
                                    <option value="{{$status->id}}" {{$product->product_accept_id == $status->id ? 'selected':'' }}>{{$status->value}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-1 col-md-6">
